...
/**
 * This class converts natural language input into machine readable math formats, such as (x)(y)
 * goes to (x)*(y) This applies to trig as well, as now sin(x) is replaced with Math.sin(x) this
 * lets other code use things like DoubleBinaryOperator.applyAsDouble() on user input
 * 
 * @author Hank O'Brien
 **/
public class InputParser {
  ...
  public static String parse(String natEq) {
    ...
    temp = temp.replace("atan2", "ATan"); // the 2 does bad things with the implicit multiplication
                                          // parser, so this temporarily gets rid of it
    ...
    temp = temp.replace("ATan", "atan2"); // addes back atan2, a later static import of Math lets it
                                          // be evaluated
  ...
  // if user input is mathematically correct, the only characters that can border the ^ sign is (,
  // ), or the digits, or trig functions
  private static String parseExponents(String natEq) {
    ...
    if (natEq.contains("^")) {
      ...
      if (natEq.charAt(i - 1) == ')') {
        ...
        // compensates for additional decrement before loop termination if firstMarker does not end
        // at 0
        firstMarker++;
      } else if (natEq.charAt(firstMarker) == var1 || natEq.charAt(firstMarker) == var2) { // case
                                                                                           // of x^3
                                                                                           // or y^3
  ...
  private static String stage2Parse(String natEq) {
    for (int i = 1; i < natEq.length(); i++) {
      if ((isDigit(natEq.charAt(i - 1)) || natEq.charAt(i - 1) == 'M'
          || isVariable(natEq.charAt(i - 1))) && natEq.charAt(i) == '(') { // 6(x) -> 6*(x)
        natEq = addMultiplicationSymbol(natEq, i);
      }
      if (natEq.charAt(i - 1) == ')'
          && (isDigit(natEq.charAt(i)) || natEq.charAt(i) == 'M' || isVariable(natEq.charAt(i)))) { // (x)6
                                                                                                    // ->
                                                                                                    // (x)*6
        natEq = addMultiplicationSymbol(natEq, i);
      }
    ...
    // fixes a small bug in the above code where a trig function was multiplied by another
    // parenthesis, like cos(x)*)
    natEq = natEq.replace("(*(", "((");
    natEq = natEq.replace(")*)", "))");
    return natEq;
  }
